...
extension Highlight {
    ...
    public static func removeFromHTMLById(withinPage page: FolioReaderPage?, highlightId: String, completion: @escaping (String?) -> Void) {
        guard let currentPage = page else { return }
        
        //WebViewMigration:
        currentPage.webView?.js("removeHighlightById('\(highlightId)')") { res in
